...
class NetatmoEnergy extends utils.Adapter {
    ...
    async _getStoredToken() {
        ...
        try {
            ...
            await this.sendRequestNotification(
                ...
                glob.ErrorNotification,
                'Get Token',
                mytools.tl('Error reading stored tokens: ', this.systemLang) + err.message,
            );
        }
    }
        ...
        switch (NetatmoRequest) {
            ...
            default:
                this.sendNotification(
                    NotificationType,
                    NetatmoRequest,
                    addText + (this.config.NoticeType == glob.NoticeTypeLong && longText != '' ? `\n${longText}` : ''),
                );
                break;
        }
    }
    ...
    async sendAPIRequest(API_URI, APIRequest, setpayload, norefresh, lastrequest) {
        ...
        if (shouldRefresh || !this.globalNetatmo_AccessToken) {
            this.log.info(
                mytools.tl('Start Token-request:', this.systemLang) + glob.blank + APIRequest + glob.blank + setpayload,
            );
            ...
            await this.getAPIRequest(API_URI, APIRequest, setpayload)
                .then(async response => {
                    switch (APIRequest) {
                        ...
                        default:
                            this.log.debug(
                                mytools.tl('API changes applied', this.systemLang) + glob.blank + APIRequest,
                            );
                    }
                ...
                .catch(async error => {
                    this.log.error(
                        mytools.tl('API request not OK:', this.systemLang) +
                            (error !== undefined && error !== null
                                ? `${glob.blank + error.error}: ${error.error_description}`
                                : ''),
                    );
    ...
    getAPIRequest(API_URI, NetatmoRequest, extend_payload) {
        ...
        this.log.debug(
            mytools.tl('Request:', this.systemLang) +
                glob.blank +
                API_URI +
                NetatmoRequest +
    ...
    async sendHomesdataAPIRequest(APIRequest, norefresh) {
        let gateway_types = await this.getValuefromDatapoint(
            glob.payload_gateway_types,
            mytools.getDP([
                this.globalAPIChannel,
                glob.Channel_homestatus,
                glob.Channel_parameters,
    ...
    async sendHomestatusAPIRequest(APIRequest, norefresh) {
        const device_types = await this.getValuefromDatapoint(
            glob.payload_device_types,
            mytools.getDP([
                this.globalAPIChannel,
                glob.Channel_homestatus,
                glob.Channel_parameters,
    ...
    async sendRoomsmeasureAPIRequest(APIRequest, norefresh) {
        ...
        let measure_payload = await this.getValuefromDatapoint(
            glob.payload_room_id,
            mytools.getDP([
                this.globalAPIChannel,
                glob.Channel_getroommeasure,
                glob.Channel_parameters,
        ...
        measure_payload =
            measure_payload +
            (await this.getValuefromDatapoint(
                glob.payload_scale,
                mytools.getDP([
                    this.globalAPIChannel,
                    glob.Channel_getroommeasure,
        ...
        if ((measure_payload.match(/&/g) || []).length == 3) {
            ...
            await this.sendRequestNotification(
                ...
                glob.WarningNotification,
                APIRequest,
                mytools.tl('Request is missing parameters', this.systemLang) +
                    mytools.tl('Actual payload:', this.systemLang) +
                    glob.blank +
                    measure_payload,
            );
        }
                            ...
                            for (const id in states) {
                                ...
                                if (id.search(searchstring) >= 0) {
                                    if (adapterstates && adapterstates.val === true) {
                                        ...
                                        const newTemp = await that.getStateAsync(
                                            mytools.getDP([actId.path, glob.Trigger_SetTemp]),
                                        );
                                        if (newTemp) {
                                            if (
                                                await that.applyActualTemp(
                                                    newTemp,
                                                    actId.parent,
                                                    NetatmoRequest,
                                                    mode,
    ...
    async applyActualTemp(newData, actParent, NetatmoRequest, mode, temp) {
        ...
        const actTemp = await this.getStateAsync(
            mytools.getDP([actParent, glob.Channel_status, glob.State_therm_setpoint_temperature]),
        );
        const actTemp_mode = await this.getStateAsync(
            mytools.getDP([actParent, glob.Channel_settings, glob.State_TempChanged_Mode]),
        );
        const actTemp_endtime = await this.getStateAsync(
            mytools.getDP([actParent, glob.Channel_settings, glob.State_TempChanged_Endtime]),
        );
        let newTemp = actTemp;
        ...
        if (
            roomnumber &&
            ((actTemp && newTemp && actTemp.val != newTemp.val) || (actTemp_mode && actTemp_mode.val != ''))
        ) {
    ...
    _myFetch(url, payload) {
        ...
        return new Promise(function (resolve, reject) {
            if (!url) {
                reject({
                    error: mytools.tl('Invalid Parameter', that.systemLang),
                    error_description: mytools.tl('Did not get url or payload!', that.systemLang),
                });
                return;
            }
            try {
    ...
    async _storeOldValue(id) {
        ...
        if (index >= 0) {
            this.mySubscribedStates.splice(index, 1);
        }
    ...
    async _subscribeStates(id) {
        ...
        if (
            (actId.state == glob.state_anticipating && actId.folder == glob.Channel_status) || //check anticipating
            ...
            (actId.state == glob.state_heating_power_request && actId.folder == glob.Channel_status)
        ) {
            //check heating request
        ...
        if (mytools.netatmoTags(obj_name) === true) {
            if (API_Request === glob.APIRequest_homesdata) {
                ...
                if (Netatmo_Path.search(searchSchedule) >= 0) {
                    await this.createNetatmoStructure(
                        mytools.getDP([Netatmo_Path, glob.State_selected]),
                        glob.State_selected,
                        false,
                        true,
                        'indicator',
        ...
        for (const object_name in obj) {
            ...
            if (obj[object_name] instanceof Object) {
                if (mytools.netatmoTags(object_name) === true) {
                    await this.getValuesFromNetatmo(
                        API_Request,
                        obj[object_name],
                        object_name,
                        myobj_selected,
                        mytools.getPrefixPath(Netatmo_Path + glob.dot) + object_name,
                        norefresh,
                    );
                } else {
                    await this.getValuesFromNetatmo(
                        ...
                        if (schedule_name) {
                            ...
                            that.globalScheduleObjects[
                                mytools.getDP([
                                    ...
                                    glob.Channel_switchhomeschedule,
                                    `${glob.APIRequest_switchhomeschedule}_${schedule_name.val.replace(
                                        /[\s[\]*,;'"&`<>\\?.^$()/]/g,
                                        '_',
                                    )}`,
                                ])
                            ...
                            const mySchedule = mytools.getSortedArray(
                                schedule_name.val,
                                schedule_id.val,
                                that.globalScheduleList,
                                that.globalScheduleListArray,
                            );
            ...
            for (const id in states) {
                if (id.search(searchRooms) >= 0) {
                    ...
                    if (room_id && room_id.val == statevalue) {
                        ...
                        // create sortet object
                        const myRooms = mytools.getSortedArray(
                            roomName !== null && roomName !== undefined ? roomName.val : room_id.val,
                            room_id.val,
                            that.globalRoomId,
                            that.globalRoomIdArray,
                        );
                        ...
                        for (const objstat_name in ObjStatus) {
                            if (!(ObjStatus[objstat_name] instanceof Object) && objstat_name != 'id') {
                                await that.createNetatmoStructure(
                                    mytools.getDP([myTargetName, glob.Channel_status, objstat_name]),
                                    objstat_name,
                                    ObjStatus[objstat_name],
                                    true,
                                    '',
                                    false,
                                    true,
                                ...
                                switch (objstat_name) {
                                    case glob.State_therm_setpoint_temperature:
                                        await that.createMyChannel(
                                            mytools.getDP([myTargetName, glob.Channel_settings]),
                                            'Change settings',
                                        );
                                        await that.createNetatmoStructure(
                                            mytools.getDP([myTargetName, glob.Channel_settings, glob.Trigger_SetTemp]),
                                        ...
                                        await that.createNetatmoStructure(
                                            mytools.getDP([
                                                myTargetName,
                                                glob.Channel_settings,
                                                glob.State_TempChanged,
                                            ]),
                                            'temperature manually changed',
                                            false,
                                            true,
                                            'indicator',
                                            false,
                                        ...
                                        await that.createNetatmoStructure(
                                            mytools.getDP([
                                                myTargetName,
                                                glob.Channel_settings,
                                                glob.State_TempChanged_Mode,
                                            ]),
                                            'The mode you are applying to this room (def=manual)',
                                            '',
                                            true,
                                            'text',
                                            true,
                                        ...
                                        await that.createNetatmoStructure(
                                            mytools.getDP([
                                                myTargetName,
                                                glob.Channel_settings,
                                                glob.State_TempChanged_Endtime,
                                            ]),
                                            'end time of the schedule mode set (seconds)',
                                            '',
                                            true,
                                            'value.time',
                                            true,
                                        ...
                                        await that.createNetatmoStructure(
                                            mytools.getDP([myTargetName, glob.Channel_settings, glob.Trigger_SetHome]),
                                            'Set the mode for this room to home',
                                            false,
                                            true,
                                            'button',
                                            true,
            ...
            for (const id in states) {
                if (id.search(searchModules) >= 0) {
                    ...
                    if (module_id && module_id.val == statevalue) {
                        const myTargetName = id.substring(0, id.length - 3);
                        await that.createMyChannel(
                            mytools.getDP([myTargetName, glob.Channel_modulestatus]),
                            'Module status',
                        );

                        const type = await that.getStateAsync(
                            mytools.getDP([myTargetName, glob.Channel_modulestatus, 'type']),
                        );
                        if (type && type.val == 'NATherm1') {
                            const deviceName = await that.getStateAsync(`${myTargetName}.name`);
                            const bridge = await that.getStateAsync(
                                mytools.getDP([myTargetName, glob.Channel_modulestatus, 'bridge']),
                            );

                            // create sortet object
                            const myDevices = mytools.getSortedArray(
                                deviceName !== null && deviceName !== undefined ? deviceName.val : type.val,
                                bridge !== null && bridge !== undefined ? bridge.val : type.val,
                                that.globalDeviceId,
                                that.globalDeviceIdArray,
                            );
                        ...
                        for (const objstat_name in ObjStatus) {
                            if (!(ObjStatus[objstat_name] instanceof Object) && objstat_name != 'id') {
                                await that.createNetatmoStructure(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, objstat_name]),
                                    objstat_name,
                                    ObjStatus[objstat_name],
                                    true,
                                    '',
    ...
    async getDateFrom1970(seconds) {
        ...
        return (
            ((adapter_time_exec !== null && adapter_time_exec !== undefined ? adapter_time_exec.val : 0) + seconds) *
            3600
        );
    }
        ...
        try {
            if (forced) {
                ...
                if (!norefresh) {
                    ...
                    if (
                        this.config.UpdateStatesIfChanged == false ||
                        (this.config.UpdateStatesIfChanged == true &&
                            (!actvalue ||
                                actvalue == null ||
                                (actvalue && actvalue != null && actvalue.val != value) ||
                                (this.config.UpdateStatesChangedTempEveryTime == true && role == 'value.temperature')))
                    ) {
                        this.log.debug(
                            `${mytools.tl('Event triggered:', this.systemLang) + glob.blank + id}: ${
                                actvalue ? ` (${actvalue.val})` : 'NULL'
                            } --> ${value}`,
                        );
    ...
    async _setTempChangedMode(id, state) {
        ...
        if (this.config.applyimmediately || (trigger && trigger.val == true)) {
            if (trigger && trigger.val == true) {
                await this.setState(trigger_id, false, true);
            }
            await this.applySingleActualTemp(
                state,
                actId.parent,
                glob.APIRequest_setroomthermpoint,
                glob.APIRequest_setroomthermpoint_manual,
                false,
            );
        } else {
            await this.compareValues(
                mytools.getDP([actId.parent, glob.Channel_status, glob.State_therm_setpoint_temperature]),
                mytools.getDP([actId.parent, glob.Channel_status, glob.State_TempChanged_Mode]),
                state.val,
                mytools.getDP([actId.path, glob.State_TempChanged]),
            );
        }
    }
    ...
    async sendMessage(id, message) {
        ...
        await this.sendRequestNotification(
            null,
            glob.SendNotification,
            mytools.tl('Warning', this.systemLang),
    ...
    onUnload(callback) {
        try {
            ...
            this.sendRequestNotification(
                null,
                glob.WarningNotification,
                `${mytools.tl('Status', this.systemLang)}:${mytools.tl('Adapter stopped', this.systemLang)}`,
                mytools.tl('Somebody stopped', this.systemLang) + glob.blank + this.namespace,
            );
            callback();
        } catch (e) {
            callback();
    ...
    _deleteSensorInterval(id, actIdValue) {
        ...
        const ActSensorIndex = this.SensorTimeouts.findIndex(
            element => element.id == id && element.value == actIdValue,
        );
        if (ActSensorIndex >= 0) {
    ...
    async _setSensorFields(id, sensor_attribs, actIdValue) {
        ...
        if (
            (sensor_attribs.action == glob.Action_home || sensor_attribs.action == glob.Action_temp) &&
            (!sensor_attribs.temp_sensor ||
                (sensor_attribs.temp_sensor == null && sensor_attribs.temp_sensor == undefined))
        ) {
            that.log.error(
                mytools.tl(
                    'Temperature sensor is missing in adapter configuration to perform action: ',
                    this.systemLang,
                ) + id,
            );
            return false;
        ...
        const setSensorState = async function (id, actIdValue, that, myHomeFolder) {
            ...
            if (valueNow && valueNow != null && valueNow != undefined && valueNow.val == actIdValue) {
                await that.setState(
                    mytools.getDP([myHomeFolder, glob.Channel_settings, glob.Trigger_SetHome]),
                    true,
                    false,
                );
        ...
        const setSensorTemp = async function (id, actIdValue, that, myHomeFolder, sensor_attribs, NewTemp) {
            ...
            if (valueNow && valueNow != null && valueNow != undefined && valueNow.val == actIdValue) {
                ...
                await that._storeOldValue(id);
                await that.compareValues(
                    mytools.getDP([myHomeFolder, glob.Channel_status, glob.State_therm_setpoint_temperature]),
                    mytools.getDP([myHomeFolder, glob.Channel_status, glob.State_TempChanged_Mode]),
                    sensor_attribs.set_value,
                    mytools.getDP([myHomeFolder, glob.Channel_settings, glob.State_TempChanged]),
                );
                await that.applySingleAPIRequest(
                    glob.APIRequest_setroomthermpoint,
                    glob.APIRequest_setroomthermpoint_manual,
                    mytools.tl('changed manually', that.systemLang),
                );
            }
        };
        ...
        const setSensorMode = async function (id, actIdValue, that, sensor_attribs) {
            ...
            if (valueNow && valueNow != null && valueNow != undefined && valueNow.val == actIdValue) {
                that.log.info(
                    mytools.tl('API Request setthermmode:', that.systemLang) + glob.blank + sensor_attribs.action,
                );
                that.applySingleAPIRequest(
                    glob.APIRequest_setthermmode,
                    sensor_attribs.action,
                    mytools.tl('Mode:', that.systemLang) + glob.blank + sensor_attribs.action,
                );
            }
        };
        ...
        const setSensorPlan = async function (id, actIdValue, that, sensor_attribs) {
            ...
            if (valueNow && valueNow != null && valueNow != undefined && valueNow.val == actIdValue) {
                that.log.info(
                    `${
                        mytools.tl('API Request swithhomeschedule:', that.systemLang) +
                        glob.blank +
                        sensor_attribs.action
                    } : ${mytools.tl('Heating Plan:', that.systemLang)}${glob.blank}${sensor_attribs.action
                        .substring(
                            sensor_attribs.action.lastIndexOf(glob.APIRequest_switchhomeschedule) +
                                glob.APIRequest_switchhomeschedule.length +
                                1,
                        )
                        .replace('_', ' ')}`,
                );
                that.applySingleAPIRequest(
                    glob.APIRequest_switchhomeschedule,
                    that.globalScheduleObjects[sensor_attribs.action],
                    mytools.tl('Heating Plan:', that.systemLang) +
                        glob.blank +
                        sensor_attribs.action
                            .substring(
                                sensor_attribs.action.lastIndexOf(glob.APIRequest_switchhomeschedule) +
        ...
        if (sensor_attribs.action == glob.Action_home) {
            myHomeFolder = sensor_attribs.temp_sensor.substring(
                0,
                sensor_attribs.temp_sensor.substring(0, sensor_attribs.temp_sensor.lastIndexOf('settings')).length - 1,
            );
            if (sensor_attribs.sensor_delay && sensor_attribs.sensor_delay > 0) {
                this.log.debug(
                    mytools.tl('Sensor action in', this.systemLang) +
                        glob.blank +
                        sensor_attribs.sensor_delay +
                        glob.blank +
                        mytools.tl('seconds!', this.systemLang) +
                        glob.blank +
                        id,
                );
                this.SensorTimeouts.push(
                    Object.assign(
                        {},
                        { id: id },
                        { value: actIdValue },
                        {
                            function: setTimeout(
                                setSensorState,
                                sensor_attribs.sensor_delay * 1000,
                                id,
                                actIdValue,
                                that,
                                myHomeFolder,
        ...
        //Internal Sensor - Set temp
            ...
            if (!isNaN(NewTemp)) {
                ...
                this.log.warn(
                    mytools.tl('No temperature stored in sensor tab! ', this.systemLang) + sensor_attribs.window_sensor,
                );
            }
        } else if (
            sensor_attribs.action == glob.APIRequest_setthermmode_schedule ||
            sensor_attribs.action == glob.APIRequest_setthermmode_hg ||
            sensor_attribs.action == glob.APIRequest_setthermmode_away
        ) {
            if (sensor_attribs.sensor_delay && sensor_attribs.sensor_delay > 0) {
                this.log.debug(
                    mytools.tl('Sensor action in', this.systemLang) +
                    ...
                    if (sensorobject.common.type != 'boolean') {
                        this.log.warn(
                            `${mytools.tl('Wrong Sensor:', this.systemLang) + glob.blank}[${id}]${
                                glob.blank
                            }${mytools.tl('You have to use a bool sensor!', this.systemLang)}`,
                        );
                    } else {
                        let sensorvalue = {};
                        ...
                        if (sensorvalue != undefined && sensorvalue != null && sensorvalue.val != oldValue) {
                            if (
                                ...
                                (sensor_attribs.window_sensor_value != true && sensorvalue.val == false)
                            ) {
    ...
    async _onStateChanged(id, state) {
        if (id.search(this.namespace) >= 0) {
            if (id.lastIndexOf(glob.dot) >= 0) {
                const actId = mytools.splitID(id);
                if (
                    actId.state == glob.state_anticipating ||
                    actId.state == glob.state_open_window ||
                    actId.state == glob.state_reachable ||
                    actId.state == glob.state_battery_state ||
                    actId.state == glob.state_heating_power_request
                ) {
                    ...
                    switch (actId.state) {
                        ...
                        case glob.state_anticipating:
                            if (
                                this.config.notify_anticipating_txt &&
                                this.config.notify_anticipating == true &&
                                this.config.notify_anticipating_txt != '' &&
                                state.val != oldValue
                            ) {
                                this.sendMessage(`${actId.parent}.name`, this.config.notify_heating_power_request_txt);
                            }
                            await this._storeOldValue(id);
                            break;
                        //Window open
                            ...
                            if (this.config.sensorsEnabled && this.config.sensorsEnabled == true) {
                                await this._sensorChanges(id, oldValue)
                                    .then(async startSyncAPI => {
                                        ...
                                        if (
                                            this.config.notify_window_open_txt &&
                                            this.config.notify_window_open == true &&
                                            this.config.notify_window_open_txt != '' &&
                                            state.val != oldValue
                                        ) {
                                            this.sendMessage(
                        ...
                        case glob.state_reachable:
                            if (
                                this.config.notify_connection_no_txt &&
                                this.config.notify_connection_no == true &&
                                this.config.notify_connection_no_txt != '' &&
                                state.val != oldValue
                            ) {
                                this.sendMessage(`${actId.parent}.name`, this.config.notify_connection_no_txt);
                            }
                            break;
                        //Battery state
                        case glob.state_battery_state:
                            if (state.val == glob.battery_low) {
                                if (
                                    this.config.notify_bat_low_txt &&
                                    this.config.notify_bat_low == true &&
                                    this.config.notify_bat_low_txt != '' &&
                                    state.val != oldValue
                                ) {
                                    this.sendMessage(`${actId.parent}.name`, this.config.notify_bat_low_txt);
                                }
                            } else {
                                if (state.val == glob.battery_medium) {
                                    if (
                                        this.config.notify_bat_medium_txt &&
                                        this.config.notify_bat_medium == true &&
                                        this.config.notify_bat_medium_txt != '' &&
                                        state.val != oldValue
                                    ) {
                                        this.sendMessage(`${actId.parent}.name`, this.config.notify_bat_medium_txt);
                                    }
                                }
                            }
                            await this._storeOldValue(id);
                        ...
                        case glob.state_heating_power_request:
                            if (
                                this.config.notify_heating_power_request_txt &&
                                this.config.notify_heating_power_request == true &&
                                this.config.notify_heating_power_request_txt != '' &&
                                state.val != oldValue
                            ) {
                                this.sendMessage(`${actId.parent}.name`, this.config.notify_heating_power_request_txt);
                ...
                if (id.lastIndexOf(glob.dot) >= 0) {
                    ...
                    switch (actId.state) {
                        ...
                        case glob.APIRequest_homesdata:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request homesdata:', this.systemLang) + glob.blank + id} - ${
                                    state.val
                                }`,
                            );
                        ...
                        case glob.APIRequest_homestatus:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request homestatus:', this.systemLang) + glob.blank + id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                        ...
                        case glob.APIRequest_getroommeasure:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request getroommeasure:', this.systemLang) + glob.blank + id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                        ...
                        case glob.APIRequest_getmeasure:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request getmeasure:', this.systemLang) + glob.blank + id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                        ...
                        case glob.Trigger_SetHome:
                            ...
                            this.setModeToHome(
                                mytools.getDP([actId.parent, glob.Channel_settings, glob.Trigger_SetHome]),
                                mytools.getDP([actId.parent, glob.Channel_settings, glob.State_TempChanged_Mode]),
                                mytools.getDP([actId.parent, glob.Channel_status, glob.State_therm_setpoint_mode]),
                            );
                            break;
                        ...
                        case glob.Trigger_SetTemp:
                            ...
                            if (!isNaN(state.val)) {
                                if (this.config.applyimmediately) {
                                    ...
                                    this.compareValues(
                                        mytools.getDP([
                                            actId.parent,
                                            glob.Channel_status,
                                            glob.State_therm_setpoint_temperature,
                                        ]),
                                        mytools.getDP([actId.parent, glob.Channel_status, glob.State_TempChanged_Mode]),
                                        state.val,
                                        mytools.getDP([actId.path, glob.State_TempChanged]),
                                ...
                                this.log.debug(
                                    mytools.tl('SetTemp: ', this.systemLang) +
                                        mytools.tl('No Number', this.systemLang) +
                                        glob.blank +
                                        state.val,
                                );
                            }
                        ...
                        case glob.Trigger_applychanges:
                            ...
                            this.setState(id, false, true);
                            this.applySingleAPIRequest(
                                glob.APIRequest_setroomthermpoint,
                                glob.APIRequest_setroomthermpoint_manual,
                                mytools.tl('changed manually', this.systemLang),
                            );
                            break;
                        ...
                        case `${glob.APIRequest_setthermmode}_${glob.APIRequest_setthermmode_schedule}`:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request setthermmode', this.systemLang)} - schedule: ${id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                            this.applySingleAPIRequest(
                                glob.APIRequest_setthermmode,
                                glob.APIRequest_setthermmode_schedule,
                                mytools.tl('schedule', this.systemLang),
                            );
                            break;
                        ...
                        case `${glob.APIRequest_setthermmode}_${glob.APIRequest_setthermmode_hg}`:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request setthermmode', this.systemLang)} - hg: ${id} - ${state.val}`,
                            );
                            this.setState(id, false, true);
                            this.applySingleAPIRequest(
                                glob.APIRequest_setthermmode,
                                glob.APIRequest_setthermmode_hg,
                                mytools.tl('frost guard', this.systemLang),
                            );
                            break;
                        ...
                        case `${glob.APIRequest_setthermmode}_${glob.APIRequest_setthermmode_away}`:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request setthermmode', this.systemLang)} - away: ${id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                            this.applySingleAPIRequest(
                                glob.APIRequest_setthermmode,
                                glob.APIRequest_setthermmode_away,
                                mytools.tl('away from home', this.systemLang),
                            );
                            break;
                        ...
                        case glob.APIRequest_synchomeschedule:
                            ...
                            this.log.debug(
                                `${mytools.tl('API Request synchomeschedule:', this.systemLang) + glob.blank + id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                        ...
                        case glob.APIRequest_createnewhomeschedule:
                            ...
                            this.log.debug(
                                `${
                                    mytools.tl('API Request createnewhomeschedule:', this.systemLang) + glob.blank + id
                                } - ${state.val}`,
                            );
                            this.setState(id, false, true);
                        ...
                        case glob.Trigger_refresh_all:
                            ...
                            this.log.debug(
                                `${mytools.tl('Refresh whole structure:', this.systemLang) + glob.blank + id} - ${
                                    state.val
                                }`,
                            );
                            this.setState(id, false, true);
                    ...
                    if (actId.state.search(glob.APIRequest_switchhomeschedule) == 0) {
                        if (state.val === true) {
                            this.setState(id, false, true);
                            this.log.debug(
                                mytools.tl('API Request swithhomeschedule:', this.systemLang) +
                                    glob.blank +
                                    this.globalScheduleObjects[id],
                            );
                            this.applySingleAPIRequest(
                                glob.APIRequest_switchhomeschedule,
                                this.globalScheduleObjects[id],
                                `'${id
                                    .substring(
                                        id.lastIndexOf(glob.dot + glob.APIRequest_switchhomeschedule) +
                                            (glob.dot + glob.APIRequest_switchhomeschedule).length +
                                            1,
                                    )
                                    .replace('_', glob.blank)}'`,
                            );
                        }
                    }
                }
                ...
                if (states && !error) {
                    for (const id in states) {
                        if (id.search(searchHomeID) >= 0) {
                            const myTargetName = id.substring(0, id.length - 3);
                            const thermmode_active = await that.getStateAsync(
                                mytools.getDP([myTargetName, 'therm_mode']),
                            );
                            if (thermmode_active && thermmode_active != null) {
                                myActiveThermMode = thermmode_active.val;
                                try {
                                    myActiveThermMode = mytools.tl(
                                        JSON.parse(conv_name_list)[myActiveThermMode],
                                        that.systemLang,
                                    );
                                } catch (e) {
                                    //no JSON
    ...
    _addStaticSensors(SensorActions) {
        ...
        SensorActions.push({
            label: mytools.tl('Scheduled', this.systemLang),
            value: glob.APIRequest_setthermmode_schedule,
        });
        SensorActions.push({
            label: mytools.tl('Frost guardian', this.systemLang),
            value: glob.APIRequest_setthermmode_hg,
        });
        SensorActions.push({
            label: mytools.tl('switchhomeschedule away from home', this.systemLang),
            value: glob.APIRequest_setthermmode_away,
        });
            ...
            that.getStates(SwitchModeChanel, async function (error, states) {
                if (states && !error) {
                    for (const id in states) {
                        const value = id
                            .substring(
                                id.lastIndexOf(glob.Channel_switchhomeschedule) +
                                    glob.Channel_switchhomeschedule.length +
                                    1,
                            )
                            .replace('_', ' ');
                        if (value) {
                            SensorActions.push({ label: value, value: id });
                        }
                    }
                ...
                if (states && !error) {
                    for (const id in states) {
                        if (id.search(searchBoolSensors) >= 0) {
                            const SensorLabel = await that.getStateAsync(
                                `${id.substring(0, id.substring(0, id.lastIndexOf('settings')).length - 1)}.name`,
                            );
                            SensorLabel
                                ? BoolArray.push({ label: `${SensorLabel.val} (${id})`, value: id })
                ...
                if (states && !error) {
                    for (const id in states) {
                        ...
                        if (converted_name && converted_name != '') {
                            myAPIRequests.push(
                                Object.assign({}, { name: converted_name, id: id, request: `${API_Request}_${name}` }),
                            );
                        } else {
                            myAPIRequests.push(
                                Object.assign(
                                    {},
                                    { name: name.replace(/_/g, ' '), id: id, request: `${API_Request}_${name}` },
                                ),
                            );
                        }
                    }
                    ...
                    for (const id in states) {
                        if (id.search(searchModules) >= 0) {
                            ...
                            if (module_id) {
                                ...
                                const battery_state = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'battery_state']),
                                );
                                const battery_level = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'battery_level']),
                                );
                                const firmware_revision = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'firmware_revision']),
                                );
                                const rf_strength = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'rf_strength']),
                                );
                                const boiler_status = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'boiler_status']),
                                );
                                const boiler_valve_comfort_boost = await that.getStateAsync(
                                    mytools.getDP([
                                        myTargetName,
                                        glob.Channel_modulestatus,
                                        'boiler_valve_comfort_boost',
                                    ]),
                                );
                                const wifi_strength = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'wifi_strength']),
                                );
                                const plug_connected_boiler = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'plug_connected_boiler']),
                                );
                                const hardware_version = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'hardware_version']),
                                );
                                const boiler_cable = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_modulestatus, 'boiler_cable']),
                                );
            ...
            that.getStates(`${that.namespace}.homes.*.rooms.*.module_ids.*`, async function (error, states) {
                if (states && !error) {
                    ...
                    for (const id in states) {
                        ...
                        if (module_id) {
                            myModule = myModules.find(element => element.val == module_id.val);
                        }
                        ...
                        if (myModule) {
                            ...
                            if (room_id) {
                                ...
                                const anticipating = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'anticipating']),
                                );
                                const open_window = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'open_window']),
                                );
                                const reachable = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'reachable']),
                                );
                                const therm_measured_temperature = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'therm_measured_temperature']),
                                );
                                const therm_setpoint_mode = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'therm_setpoint_mode']),
                                );
                                const therm_setpoint_temperature = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'therm_setpoint_temperature']),
                                );
                                const heating_power_request = await that.getStateAsync(
                                    mytools.getDP([myTargetName, glob.Channel_status, 'heating_power_request']),
                                );
                                const Set_Temp = mytools.getDP([
                                    myTargetName,
                                    glob.Channel_settings,
                                    glob.Trigger_SetTemp,
                                ]);
                                const Set_Mode = myTargetName;
                                const Set_Mode_Home = mytools.getDP([
                                    myTargetName,
                                    glob.Channel_settings,
                                    glob.Trigger_SetHome,
                                ]);

                                const myHomeFolder = id.substring(
                                    0,
                                    id.substring(0, id.lastIndexOf('rooms')).length - 1,
                                );
    ...
    async _authenticate(args, obj) {
        ...
        if (!args.redirect_uri || !args.code) {
            return;
        }
        ...
        if (args.access_token) {
            ...
            const setpayload = `code=${args.code}&redirect_uri=${args.redirect_uri}&grant_type=authorization_code${
                glob.payload_client_id
            }${this.config.ClientId}${glob.payload_client_secret}${this.config.ClientSecretID}${
                glob.payload_scope
            }${glob.OAuthScope}`;
            this._setTokenIntervall(false);
                ...
                .then(async tokenvalues => {
                    ...
                    obj.callback &&
                        this.sendTo(
                            obj.from,
                            obj.command,
                            { result: `${mytools.tl('Tokens updated successfully.', this.systemLang)}` },
                            obj.callback,
                        );
                    await this._setTokenIntervall(true);

                    this.log.info(
                        mytools.tl('Update data in adapter configuration ... restarting ...', this.systemLang),
                    );
                ...
                .catch(async error => {
                    ...
                    obj.callback &&
                        this.sendTo(
                            ...
                            obj.command,
                            {
                                error: `${mytools.tl('Error getting new tokens from Netatmo: ', this.systemLang)} ${error} ${mytools.tl('. Please try again.', this.systemLang)}`,
                            },
                            obj.callback,
                        );
                });
            ...
            switch (local_command) {
                ...
                case glob.HomeMode:
                    if (obj.callback) {
                        ...
                        setData = obj.message;
                        this.setState(
                            mytools.getDP([setData.folder, glob.Channel_settings, glob.Trigger_SetHome]),
                            true,
                            false,
                        );
                        ...
                        try {
                            ...
                            if (inst === '') {
                                this.sendTo(
                                    obj.from,
                                    obj.command,
                                    [{ label: 'All Receiver', value: 'allTelegramUsers' }],
                                    obj.callback,
                                );
                            } else {
                ...
                case glob.SendTestNotification: {
                    if (obj.callback) {
                        this.sendNotification(
                            glob.InfoNotification,
                            'ioBroker',
                            mytools.tl('This is a test message!', this.systemLang),
                        );
                        this.sendTo(
                            obj.from,
                            obj.command,
                            { result: mytools.tl('Test message has been sent!', this.systemLang) },
                            obj.callback,
                        );
                    }
                    break;
                ...
                case glob.GetSetTempSensors:
                    if (obj.callback) {
                        this._getSensors(
                            'homes\\.\\d+\\.rooms\\.\\d+\\.settings.SetTemp',
                            '.homes.*.rooms.*.settings.SetTemp',
                        )
                            .then(MySensorsArray => {
                ...
                case glob.GetOAuthStartLink: {
                    ...
                    this.log.debug(
                        mytools.tl('Received OAuth start message: ', this.systemLang) + JSON.stringify(auth_args),
                    );
                    ...
                    if (!auth_args.redirect_uri_base.endsWith('/')) {
                        auth_args.redirect_uri_base += '/';
                    }
                    auth_args.redirect_uri = `${auth_args.redirect_uri_base}oauth2_callbacks/${this.namespace}/`;
                    delete auth_args.redirect_uri_base;

                    this.log.debug(
                        mytools.tl('Get OAuth start link data: ', this.systemLang) + JSON.stringify(auth_args),
                    );
                ...
                case glob.GetOAuthCallback: {
                    ...
                    if (!auth_args.state || !auth_args.code) {
                        this.log.warn(
                            mytools.tl('Error on OAuth callback: ', this.systemLang) + JSON.stringify(auth_args),
                        );
                        if (auth_args.error) {
                            obj.callback &&
                                this.sendTo(
                                    obj.from,
                                    obj.command,
                                    { error: `Netatmo error: ${auth_args.error}. Please try again.` },
                                    obj.callback,
                                );
                        } else {
                            obj.callback &&
                                this.sendTo(
                                    ...
                                    obj.command,
                                    {
                                        error: `Netatmo invalid response: ${JSON.stringify(auth_args)}. Please try again.`,
                                    },
                                    obj.callback,
                                );
                        }

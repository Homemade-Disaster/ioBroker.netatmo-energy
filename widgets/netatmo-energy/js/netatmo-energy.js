...
vis.binds.netatmobasic = {
	...
	slider: function (el, wid, view, data, options) {
		...
		const oid2 = $this.attr('data-oid2');
		const oid_val = 0;
		const oid2_val = 0;
		const woid = $this.attr('data-oid-working');
		const woid2 = $this.attr('data-oid2-working');
		...
		const settings = $.extend(
			{
				...
				slide: function (/** @type {any} */ e, /** @type {{ values: any[]; value: number; }} */ ui) {
				...
				stop: function (/** @type {any} */ e, /** @type {{ values: any[]; value: number; }} */ ui) {
					if (options.submitmethod === 'stop') {
						if (oid2) {
							if (options.inverted) {
								ui.values[0] = settings.max - ui.values[0] + settings.min;

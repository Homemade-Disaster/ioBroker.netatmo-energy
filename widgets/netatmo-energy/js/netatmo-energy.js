...
function buildtemptext(data) {
	...
	if (data.position_target) {
		txtlines[1] = new Array(data.position_target,'<span ' + color_stxt + '>' + valve_soll + temp_soll + '</span>');
	}
	if (data.position_now) {
		txtlines[2] = new Array(data.position_now,'<span ' + color_vtxt + '>' + valve_now  + temp_now  + '</span>');
	}
	if (data.position_actual) {
		txtlines[3] = new Array(data.position_actual,'<span ' + color_itxt + '>' + valve_ist  + temp_ist + '</span>');
	}
	txtlines.sort();
...
function buildwindowopen(data) {
	...
	if (data.windowopen_oid && vis.states.attr(data.windowopen_oid + '.val') === true && ( data.show_windowopen == true || data.show_windowopen === 'true')) {
		show_image = '<img class="netatmo-pic-window" style="width: ' + data.size_window + 'px; height: auto;" src="' + icon_img + '" alt="window open"></img>';
	}
	return (show_image);
}
...
function buildreachable(data) {
	...
	if (data.reachable_oid && vis.states.attr(data.reachable_oid + '.val') === false && ( data.show_reachable == true || data.show_reachable === 'true')) {
		show_image = '<img class="netatmo-pic-reachable" style="width: ' + data.size_reachhable + 'px; height: auto;" src="' + icon_img + '" alt="reachable"></img>';
	}
	return (show_image);
}
...
function buildanticipating(data) {
	...
	if (data.anticipating_oid && vis.states.attr(data.anticipating_oid + '.val') === true && ( data.show_anticipating == true || data.show_anticipating === 'true')) {
		show_image = '<img class="netatmo-pic-anticipating" style="width: ' + data.size_anticipating + 'px; height: auto;" src="' + icon_img + '" alt="anticipating"></img>';
	}
	return (show_image);
}
							...
							case 'value.roomname':
								...
								if (position >= 0) {
									if (!vis.views[view].widgets[wid].data.refr_oid) {
										...
										vis.views[view].widgets[wid].data.refr_oid = result[r].substring(0, position + 1) + 'energyAPP.trigger.refresh_structure';
										vis.widgets[wid].data.refr_oid = result[r].substring(0, position + 1) + 'energyAPP.trigger.refresh_structure';
									}
